...
void AcHlp_GetSectionBaseLen(unsigned char* imagestart, const char* sectionname, DWORD* sectiondata, DWORD* sectionlen, DWORD *virtualaddr)
{
	DWORD dwSectionSize;
	DWORD dwSectionBase;
	DWORD dwVirtualAddr;
...
int AcHlp_PerformRelocation(unsigned char* imagestart, DWORD imagebase) //Only for .text right now
{
	DWORD i, num_items;
	DWORD_PTR diff;
	...
	DWORD reloc_size = 0;
	DWORD reloc_virt;
    PROTECTSTRING(".reloc", sbuf);
	AcHlp_GetSectionBaseLen(imagestart, sbuf, (DWORD*)&r, &reloc_size, &reloc_virt);
	...
	AcHlp_GetSectionBaseLen(imagestart, sbuf, &text, &text_size, &text_virt);

	DWORD rdata;
	DWORD rdata_size = 0;
	DWORD rdata_virt;
    PROTECTSTRING(".rdata", sbuf);
	AcHlp_GetSectionBaseLen(imagestart, sbuf, &rdata, &rdata_size, &rdata_virt);
	...
	for (; r<r_end; r=(IMAGE_BASE_RELOCATION*)((DWORD_PTR)r + r->SizeOfBlock))
	{
			...
			//		printf("offset: %x\n", offset);
					...
					else if(rdata_size > 0 && offset >= rdata_virt && offset < rdata_virt + rdata_size)
					{
						offset -= rdata_virt;
						*((DWORD_PTR*)(rdata + offset)) += diff;
					}
					break;
				default:
					return 0;
