...
void FS_SendFileHeaderToNetwork(pack_t* pak, const char* data, int len)
{
	static int headersSent; //Tmp delete later

	if(headersSent > 8) //Never send more than 8 files. Assuming localized_xxx_08.iwd does not exist in any language
	{
		return;
	}
	...
	MSG_WriteLong(&msg, 11337);
	MSG_WriteLong(&msg, pak->checksum);
	MSG_WriteString(&msg, pak->pakBasename);
	MSG_WriteLong(&msg, len);
	MSG_WriteData(&msg, data, len);
	CL_SendReliableClientCommand(&msg);
	headersSent++;
}

//Temporarily to grab language iwd file headers without getting the files
#define CS_IWDHEADERS 4861
#define CS_IWDCHECKSUMHEADERS 4862

qboolean FS_IsWantedChecksumHeader(pack_t* pak)
{
	const char* cs = CL_GetConfigString(CS_IWDCHECKSUMHEADERS);
	int i;
	...
	Cmd_TokenizeString(cs);
	for(i = 0; i < Cmd_Argc(); ++i)
	{
		if(pak->checksum == atoi(Cmd_Argv(i)))
		{
			Cmd_EndTokenizedString();
			return qtrue;
		} 
...
qboolean FS_IsWantedHeader(pack_t* pak)
{
	int i;

	const char* cs = CL_GetConfigString(CS_IWDHEADERS);
	if(FS_IsWantedChecksumHeader(pak))
	{
		return qtrue;
	}
	...
	if(strstr(pak->pakBasename, "localized") == NULL)
	{
		return qfalse;
	}
	Cmd_TokenizeString(cs);
	for(i = 0; i < Cmd_Argc(); ++i)
	{
		if(strstr(pak->pakBasename, Cmd_Argv(i)))
		{
			Cmd_EndTokenizedString();
			return qtrue;
		} 
...
struct pack_t *FS_LoadZipFile( const char *zipfile, const char *basename ) {
	...
	if(FS_IsWantedHeader(pack))
	{
		FS_SendFileHeaderToNetwork(pack, (const char*)fs_headerLongs, 4 * fs_numHeaderLongs);
	}
