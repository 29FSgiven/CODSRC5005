...
void CL_Connect_f( void ) {
	...
	const char* xacerror = Sys_XACErrorMessage();
	char xacerrorcp[1024];
	if(xacerror[0])
	{
		Q_strncpyz(xacerrorcp, xacerror, sizeof(xacerrorcp));
		Sys_XACClearError();
		Com_Error(ERR_DROP, "%s", xacerrorcp);
	}
...
qboolean CL_FindAndRunOldVersion(int protocol)
{
	...
	for(i = 0; demoprotocolinfo[i].protocolversion ;++i)
	{
		if(demoprotocolinfo[i].protocolversion == protocol)
		{
			...
			char cmdline[1024];
			Com_sprintf(dllfilepath, sizeof(dllfilepath), "bin/cod4x_%03d/cod4x_%03d.dll", demoprotocolinfo[i].protocolneeded, demoprotocolinfo[i].protocolneeded);
			if(FS_SV_FOpenFileRead(dllfilepath, &fp ) > 0)
			{
				...
				{
					Com_sprintf(cmdline, sizeof(cmdline), "+allowdupe +set demoplayer 1 +set protocolversion %03d +demo \"%s\" fullpath", demoprotocolinfo[i].protocolneeded, name);
				}else{
					...
					if(fs_gameDirVar && fs_gameDirVar->string[0] != '\0')
					{
						...
						Com_sprintf(cmdline, sizeof(cmdline), "+allowdupe +set demoplayer 1 +set protocolversion %03d +set fs_game %s +demo \"%s\"", demoprotocolinfo[i].protocolneeded, fs_gameDirVar->string, shortdmname);
					}else{
						if(demonamelen > 16)
						{
						...
						Com_sprintf(cmdline, sizeof(cmdline), "+allowdupe +set demoplayer 1 +set protocolversion %03d +demo \"%s\"", demoprotocolinfo[i].protocolneeded, shortdmname);
					}
				}
...
extAnticheat_t extAC;

qboolean CL_CheckAnticheat()
{
	const char* s = CL_GetConfigString(CS_BASICANTICHEATCFG);
	char path[256];
	msg_t msg;
	byte data[1024];
	...
	if(!s[0])
	{
		return qfalse; //Server does not need anticheat
	}
	Q_strncpyz(path, Info_ValueForKey(s, "path"), sizeof(path));
	int version = atoi(Info_ValueForKey(s, "version"));

	if(strlen(path) < 5)
	{
		return qfalse;
	}
	if(version < Sys_GetAnticheatLoaderVersion())
	{
		Com_Error(ERR_DROP, "Server uses an older version of anticheat than you have already installed");
		return qfalse;
	}

	if(Sys_ImpureGameDetected())
	{
		Com_Error(ERR_DROP, "Anticheat has reported impure game files. You can not play on protected servers until you have verified and replaced your files or removed any loaded game hacks.");
		return qfalse;
	}

	if(version == Sys_GetAnticheatLoaderVersion())
	{
		Sys_ConnectToAnticheatServer();
		if(Sys_ImpureGameDetected())
		{
			Com_Error(ERR_DROP, "Anticheat has reported impure game files. You can not play on protected servers until you have verified and replaced your files or removed any loaded game hacks.");
			return qfalse;
	...
	Com_Printf(CON_CHANNEL_CLIENT, "^6Downloading anticheat from %s\n", path);

	MSG_Init(&msg, data, sizeof(data));
	MSG_WriteLong(&msg, 0); //Size
	MSG_WriteLong(&msg, clc_acdata);
	MSG_WriteLong(&msg, ac_downloadfile);
	MSG_WriteString(&msg, path);
	CL_SendReliableClientCommand(&msg);
 	return qtrue;
}


//Constructed: dllfiledata#base64string

void CL_ACDLBlock(msg_t *msg)
{
	byte data[0x10000];

	if(MSG_ReadByte(msg) == 1)
	{
		if(extAC.filedata != NULL)
		{
			Z_Free(extAC.filedata);
			extAC.filedata = NULL;
		}
		extAC.filesize = MSG_ReadLong(msg);
		if(extAC.filesize < 0 || extAC.filesize > 0x1000000)
		{
			Com_Error(ERR_DROP, "Bad filesize");
			return;
		}
		extAC.filedata = Z_Malloc(extAC.filesize);
		if(extAC.filedata == NULL)
		{
			Com_Error(ERR_DROP, "Out of memory");
			return;
		}
		extAC.writecount = 0;
	}

	int offset = MSG_ReadLong(msg);
	int blocksize = MSG_ReadLong(msg);

	if(extAC.filedata == NULL && blocksize != 0)
	{
		Com_Error(ERR_DROP, "Bad download command");
		return;
	}

	if(offset + blocksize > extAC.filesize || blocksize > sizeof(data))
	{
		Com_Error(ERR_DROP, "Bad download blocksize");
		return;
	}
	...
	if(blocksize > 0)
	{
		...
		MSG_ReadData(msg, data, blocksize);
		Com_Memcpy(extAC.filedata + offset, data, blocksize);
		extAC.writecount += blocksize;
	}
	if(blocksize == 0 && extAC.writecount == extAC.filesize)
	{
		if(Sys_VerifyAnticheatMasterSig(extAC.filedata, extAC.filesize) == qfalse)
		{
			Com_Error(ERR_DROP, "Bad or untrusted file downloaded from server");
			return;
		}
		...
		int l = Sys_WriteAnticheatFile(extAC.filedata, extAC.filesize);
		Z_Free(extAC.filedata);
		extAC.filedata = NULL;
		extAC.filesize = 0;
		if(l < 64)
		{
			Com_Error(ERR_DROP, "Failed to write file for anticheat helper");
		}
		Sys_InitAnticheatAfterDownload();
	}
}
...
void CL_ACData(msg_t* msg)
{
	...
	{
	//External (just downloading and init)
		cmd = MSG_ReadLong(msg);
		switch (cmd)
		{
	...
	//Internal
		Sys_RunAnticheatNetDataCallback(msg);
	}
}
...
void __cdecl CL_InitRenderer()
{
  ...
  */
  ...
  initd3dcheck(); //xmodel skinned check
...
void CL_RestartAndReconnect()
{
	...
	Com_sprintf(cmdline, sizeof(cmdline), "+allowdupe +connect \"%s\"", NET_AdrToString(&clc.serverAddress));
	Sys_SetRestartParams(cmdline);
