...
public:
	...
	bool ParseVtable(uint32_t** classptr, safeimportaccess_t* fields, uint32_t* semiclassbase);
...
SteamAPI_Main::SteamAPI_Main(  const wchar_t* dllpath, AppId_t parentAppID, AppId_t gameId, const char* productName, bool overlayrenderer)
{
	...
	/* Unofficial */
#if 1
	IClientEngine* iClientEngine = (IClientEngine*)CreateInterfaceFunction("CLIENTENGINE_INTERFACE_VERSION005", &retval);
#endif
	if(iSteamClient == nullptr || iClientEngine == nullptr)
	{
	...
	if(!ParseVtable((uint32_t**)iClientEngine->GetIClientUtils(hSteamPipe, nullptr), iClientUtilsFields, (uint32_t*)&iClientUtils))
	{
		ShutdownAPI();
...
#if 1
  if(!ParseVtable((uint32_t**)iClientEngine->GetIClientUser(hSteamUser, hSteamPipe, nullptr), iClientUserFields, (uint32_t*)&iClientUser))
	{
		ShutdownAPI();
...
bool SteamAPI_Main::ParseVtable(uint32_t** classptr, safeimportaccess_t* fields, uint32_t* semiclassbase)
{
    int i, j;
	const char* funcname;
...
extern "C" {
	...
	bool SteamAPI_InitX(const wchar_t* dllpath, AppId_t parentAppID, AppId_t gameId, const char* modName, bool overlayrenderer)
	{
		...
		if(steamInitThread == nullptr)
		{
			...
			if(WaitForSingleObject(steamInitThread, 60000) == WAIT_TIMEOUT)
			{
				TerminateThread(steamInitThread, -1);
				steamApiCtx = nullptr;
