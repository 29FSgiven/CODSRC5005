...
static void (*AC_StartDump_proc)(const char*);
static void (*AC_FinishDump_proc)();
static void (*AC_WriteSym_proc)(DWORD, DWORD, int);

static void AC_StartDump()
{
	__asm__("mov %%eax, %%eax\n"
			"mov %%edx, %%edx\n"
			"mov %%ecx, %%ecx\n"
	...
	HMODULE achmod = LoadLibraryA("ac_dumper.dll");
	if(achmod == NULL)
	{
		exit(0xc3);
	}
	AC_StartDump_proc = (void (*)())GetProcAddress(achmod, "HlStart");
	AC_FinishDump_proc = (void (*)())GetProcAddress(achmod, "HlFini");
	AC_WriteSym_proc = (void (*)())GetProcAddress(achmod, "HlGet");

	char versionstring[1024];

	Com_BuildVersionString(versionstring, sizeof(versionstring));

	AC_StartDump_proc( versionstring );
...
static void AC_WriteSymbol(DWORD addr, DWORD symbol, int rel)
{
		__asm__("mov %%eax, %%eax\n"
			"mov %%edx, %%edx\n"
			"mov %%ecx, %%ecx\n"
	...
	AC_WriteSym_proc(addr, symbol, rel);
...
void WriteSymbol(DWORD addr, void* symbol)
{
	...
	*paddr = (uint32_t)symbol;
	AC_WriteSymbol(addr, (DWORD)symbol, 0);
}
...
void WriteSymbolRel(DWORD addr, void* symbol)
{
	...
	*paddr = (uint32_t)width;
	AC_WriteSymbol(addr, (DWORD)symbol, 1);
}
