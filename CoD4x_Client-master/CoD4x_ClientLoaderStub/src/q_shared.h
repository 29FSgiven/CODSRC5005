...
#define MASTER_SERVER_NAME	"cod4master.cod4x.me"
#endif
...
#define zed_debug(M, ...)
#else
#define zed_debug(M, ...) Com_DPrintf( "%s:%d: " M "\n", __FILE__, __LINE__, ##__VA_ARGS__)
#endif

#define clean_errno() (errno == 0 ? "None" : strerror(errno))

#define log_err(M, ...) Com_PrintError( "(%s:%d: errno: %s) " M "\n", __FILE__, __LINE__, clean_errno(), ##__VA_ARGS__)

#define log_warn(M, ...) Com_PrintWarning( "(%s:%d: errno: %s) " M "\n", __FILE__, __LINE__, clean_errno(), ##__VA_ARGS__)

#define log_info(M, ...) Com_DPrintf( "(%s:%d) " M "\n", __FILE__, __LINE__, ##__VA_ARGS__)
...
#define	MAX_STRING_TOKENS	64	// max tokens resulting from Cmd_TokenizeString
